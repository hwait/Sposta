            ...
            if 'evid' in request.GET and 'sn' in request.GET:
                ...
                if isls:
                    ...
                    if gid>0:
                        ...
                        currentgameend=LSPoint.objects.filter(gid=nextgame).order_by('dtc')[0].dtc
                        ...
                        shapespoint.append(dict(type='rect',yref='paper',x0=pts,y0=0,x1=currentgamestart,y1=1,fillcolor='#ffccbc', opacity=0.3,line={'width': 0}))
                        shapespoint.append(dict(type='rect',yref='paper',x0=currentgameend,y0=0,x1=pte,y1=1,fillcolor='#ffccbc', opacity=0.3,line={'width': 0}))
                        if lsev.reversed==0:
                        ...
                        annospoint.append(dict(x=currentgamestart-timedelta(seconds=10),y=1,xref='x',yref='paper', showarrow=False,text=prevgametxt, textangle=-90, font=dict(family='verdana', size=14, color='#1f77b4'),opacity=0.7 ))
                        ...
                        for point in currentpoints:
                            shapespoint.append(dict(type='line',yref='paper',x0=point.dtc,y0=0,x1=point.dtc,y1=1,line=linestylepoints))
                            annospoint.append(dict(x=point.dtc+timedelta(seconds=3),y=1,xref='x',yref='paper', showarrow=False,text=self.getPointText(point.sc1, point.sc2,lsev.reversed), textangle=-90,  font=dict(family='verdana', size=11, color='#1f77b4'),opacity=0.8 ))
                        shapespoint.append(dict(type='line',yref='paper',x0=currentgamestart,y0=0,x1=currentgamestart,y1=1,line=linestylegames))
                    ...
                    if(sn==0): # not IP
                        ...
                        gameshapes.append(dict(type='line',yref='paper',x0=ts,y0=0,x1=ts,y1=1,line=linestylegames))
                        gameshapes.append(dict(type='rect',yref='paper',x0=ts,y0=0,x1=te,y1=1,fillcolor='#ffccbc', opacity=0.3,line={'width': 0}))
                        if isbf:
                        ...
                        gameshapes.append(dict(type='line',yref='paper',x0=curgames,y0=0,x1=curgames,y1=1,line=linestylegames))
                        ...
                        gameshapes.append(dict(type='line',yref='paper',x0=ge,y0=0,x1=ge,y1=1,line=linestylegames))                        
                        gameshapes.append(dict(type='rect',yref='paper',x0=ge,y0=0,x1=te,y1=1,fillcolor='#ffccbc', opacity=0.3,line={'width': 0}))
                    else: # Last set
                        ...
                        if gid>0: 
                            ...
                            gameshapes.append(dict(type='line',yref='paper',x0=te,y0=0,x1=te,y1=1,line=linestylegames))
                            gameshapes.append(dict(type='rect',yref='paper',x0=ts,y0=0,x1=te,y1=1,fillcolor='#ffccbc', opacity=0.3,line={'width': 0})) 
                            te=endset[0].dtc
                        if isbf:
                        ...
                        for g in games:
                            ...
                            if gid==0:
                                ...
                                if not (sn<setsCount and c==len(games)-1):
                                    annos.append(dict(x=games[c].dtc+timedelta(seconds=30),y=1,xref='x',yref='paper', showarrow=False,text=gametxt, textangle=-90,  font=dict(family='verdana', size=12, color='#1f77b4'),opacity=0.8 ))
                                if g.serve==1 and c<len(games)-2 and c>0:
                                    gameshapes.append(dict(type='rect',yref='paper',x0=g.dtc,y0=0,x1=games[c+1].dtc,y1=1,fillcolor='#d3d3d3', opacity=0.3,line={'width': 0}))
                            c+=1
                    if (sn==0):
                        title='Not In-Play'
                    else:
...
class GameView(View):
    def get(self, request):
        params=dict()
        if request.method == 'GET':
            if 'evid' in request.GET and 'sn' in request.GET and 'id' in request.GET:
                setsCount=1
                ...
                params['player1']=mevent.p1
                params['player2']=mevent.p2
                lsev=LSEvent.objects.prefetch_related('lsgame_set').filter(meid=mevent.meid)[0]
                bfev=BFEvent.objects.prefetch_related('bfodds_set').filter(meid=mevent.meid)[0]
                currgame=LSGame.objects.prefetch_related('lspoint_set').get(gid=id)
                prevgame=LSGame.objects.prefetch_related('lspoint_set').filter(eid=lsev, dtc__lt=currgame.dtc).latest('dtc')[0]
                ...
                if isls:
                    ...
                    if(sn==0): # not IP
                        ...
                        games=LSGame.objects.prefetch_related('lspoint_set').filter(eid=lsev, dtc__gte=ts, setn__lt=6).order_by('dtc')
                        if isbf:
                            bfchanges=BFOdds.objects.filter(eid=bfevent[0], dtc__gte=ts,b1odds__gt=0,b2odds__gt=0,l1odds__gt=0,l2odds__gt=0).order_by('dtc')
                    params['games']=[]
                    gameshapes=[]
                    annos=[]
                    if len(games)>0:
